...
public class RequestCreator {
  ...
  //注意deferred只对ImageView有效，如果是一个普通的Target使用会直接抛出异常
   ...
   * 注意这个方法只对ImageView有效，因为在有的情况下需要等待ImageView绘制完成
   */
   ...
   * 同步获取请求对应的Bitmap，注意该方法必须在子线程中执行，这个和UIL是一致的
   * <p>
   ...
   * 预加载，根本不关心载体的请求，注意在这种情况下如果不使用内存和硬盘缓存是没有什么意义的
   */
  ...
  public void into(@NonNull Target target) {
    ...
    //注意"  "这种空字符串在Picasso中会直接抛出异常。。。
    if (!data.hasImage()) {//检查当前请求是否有明确的uri或者资源id
      //这里稍微注意一下，如果当前Target之前就有任务进行当中，那么这里会取消掉之前的所有任务
  ...
  public void into(ImageView target, Callback callback) {
    ...
    if (deferred) {//如果采用fit()模式
      ...
      //如果当前ImageView绘制还没结束或者又被要求强行进行布局绘制
      if (width == 0 || height == 0 || target.isLayoutRequested()) {
        if (setPlaceholder) {
          setPlaceholder(target, getPlaceholderDrawable());
        }
        //只有当ImageView可以正确获取高宽之后才发起请求
    ...
    if (shouldReadFromMemoryCache(memoryPolicy)) {//是否使用内存缓存
      //尝试从内存缓存中获取数据
      Bitmap bitmap = picasso.quickMemoryCacheCheck(requestKey);
      if (bitmap != null) {//击中内存缓存
        picasso.cancelRequest(target);//结束当前任务
        setBitmap(target, picasso.context, bitmap, MEMORY, noFade, picasso.indicatorsEnabled);
        if (picasso.loggingEnabled) {
          log(OWNER_MAIN, VERB_COMPLETED, request.plainId(), "from " + MEMORY);
        }
        if (callback != null) {//回调获取成功
          callback.onSuccess();
        }
        return;
      }
  ...
  private Request createRequest(long started) {
    ...
    //默认是不会进行请求的转换，除非在Picasso的Build当中自定义
    Request transformed = picasso.transformRequest(request);
    if (transformed != request) {
      // If the request was changed, copy over the id and timestamp from the original.
      // 如果在transfromRequest中进行的是深拷贝或者new操作，需要重新复制id和时间
      transformed.id = id;
